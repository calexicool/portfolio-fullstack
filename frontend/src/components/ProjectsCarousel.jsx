...
export default function ProjectsCarousel({
  ...
  const setHover = (v) => {
    if (hoverLocalRef) hoverLocalRef.current = v;
  };
  ...
  const idxNear = React.useCallback(() => {
    const el = listRef.current,
      cw = cardWRef.current;
    if (!el || !cw) return 0;
    return Math.round(el.scrollLeft / cw);
  }, []);

  const startSmoothTo = React.useCallback(
    (leftTarget) => {
      const el = listRef.current;
      if (!el) return;
  ...
  React.useEffect(() => {
    ...
    // стили (скрытие скроллбара, курсор)
    ...
    const ro = new ResizeObserver(() => {
      const i = idxNear();
      measure();
      el.scrollLeft = i * (cardWRef.current || 1);
      keepLoop();
    });
    ro.observe(el);
    ...
    const onWheel = (e) => {
      // «заглушаем» внешний горизонтальный скролл секций
      ...
      const base = idxNear(),
        cw = cardWRef.current || 1;
      if (wheelState.current.state === 0) {
        wheelState.current.state = 1;
        wheelState.current.sign = sign;
        el.style.scrollSnapType = "none";
  ...
  const onPointerDown = (e) => {
    ...
    const interactive = e.target.closest(
      'a,button,input,textarea,select,[data-no-drag],[contenteditable="true"]'
    );
        ...
        {[0, 1, 2].map((copy) => (
          <React.Fragment key={copy}>
            {projects.map((p) => (
              <div
                key={`${copy}-${p.id}`}
                className="carousel-item snap-start shrink-0 basis-[80vw] md:basis-[60vw] lg:basis-[45vw]"
              >
                <article className="group relative h-[70svh] overflow-hidden rounded-3xl border">
                  <EditableImage
                    ...
                    height={
                      ...
                      Math.min(
                        520,
                        Math.max(240, Math.round(window.innerHeight * 0.7))
                      )
                    }
                    widthPct={100}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <p className="mt-2 opacity-80">
                      <EditableText
                        ...
                        value={p.summary}
                        onChange={(v) =>
                          onMutateProject(p.id, { summary: v })
                        }
                      />
                    </p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(p.tags || []).map((t, i) => (
                        <span
                          key={i}
                          className="group/tag relative rounded-full bg-neutral-900/10 px-2 py-1 text-xs dark:bg-white/10"
                        >
                          <EditableText
                            admin={admin}
                            value={t}
                            onChange={(v) => {
                              const next = [...(p.tags || [])];
                              next[i] = v;
                              onMutateProject(p.id, { tags: next });
                            }}
                          />
                          {admin && (
                            <button
                              ...
                              className="absolute -right-2 -top-2 hidden h-5 w-5 items-center justify-center rounded-full bg-rose-500 text-white group-hover/tag:flex"
                            >
                              ×
                            </button>
                          )}
                      ...
                      {admin && (
                        <button
                          type="button"
                          onClick={() =>
                            onMutateProject(p.id, {
                              tags: [...(p.tags || []), "Tag"],
                            })
                          }
                          className="rounded-full border px-2 py-1 text-xs opacity-70 hover:opacity-100"
                        >
                          + тег
                        </button>
                      )}
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                      <a
                        href={`#project-${p.id}`}
                        className="inline-flex items-center gap-2 rounded-xl bg-white/80 px-3 py-2 text-sm shadow backdrop-blur transition hover:scale-105 dark:bg-neutral-900/80"
                      >
                      ...
                      {isHttp(p.link) && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-2 rounded-xl border bg-white/60 px-3 py-2 text-sm backdrop-blur dark:bg-neutral-900/60"
                        >
                          {strings.openBtn}
                        </a>
                      )}
      ...
      <div className="pointer-events-none absolute -left-2 -right-2 top-1/2 flex -translate-y-1/2 justify-between gap-2">
        <button
          ...
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            const cw = cardWRef.current || window.innerWidth * 0.6;
            listRef.current?.scrollBy({ left: -cw, behavior: "smooth" });
          }}
          className="pointer-events-auto rounded-full bg-white/70 p-2 shadow backdrop-blur hover:scale-105 dark:bg-neutral-900/70"
        >
          <ChevronLeft />
        </button>
        <button
